...
function renderSections() {
  ...
  SECTIONS.forEach(sec => {
    const useGroupTotal = typeof sec.totalBudgetCNY === 'number';
    let budgetCNY = useGroupTotal
      ? sec.totalBudgetCNY
      : sec.items.reduce((s, i) => s + toCNY(i.budget, i.cur), 0);
    const icon = SECTION_ICONS[sec.id] || '';
    html += `<div class="section${useGroupTotal ? ' no-item-budget' : ''}" data-section="${sec.id}">
    ...
    sec.items.forEach(item => {
      ...
      // 整组预算模式：不展示单项预算与占位预算
      const budgetLabel = useGroupTotal ? '' : fmtByCur(item.budget, item.cur);
      const placeholder = useGroupTotal ? '0' : item.budget;
      html += `<div class="item-row">
        <span class="item-name">${item.name}<span class="cur-pill">${curLabel}</span></span>
        <span class="item-budget">${budgetLabel}</span>
        <input class="item-input" type="number" min="0" step="1" placeholder="${placeholder}" data-item="${item.id}" data-cur="${item.cur}" data-budget="${item.budget}" />
...
function recalc() {
  ...
  SECTIONS.forEach(sec => {
    ...
    // 整组总预算：若 section 配了 totalBudgetCNY，则用它替代各 item.budget 之和
    const groupTotalBudgetCNY = (typeof sec.totalBudgetCNY === 'number') ? sec.totalBudgetCNY : null;
    sec.items.forEach(item => {
      ...
      if (raw !== null && !isNaN(raw)) {
        ...
        anyFilled = true;
        // 若该组用整组预算（无单项预算），不显示单项 variance
        if (groupTotalBudgetCNY != null) {
          varEl.textContent = '';
    ...
    // 用整组预算覆盖 section 的预算显示
    if (groupTotalBudgetCNY != null) secBudget = groupTotalBudgetCNY;
    if (isSavings) {
      savingsTargetCNY = (groupTotalBudgetCNY != null)
        ? groupTotalBudgetCNY
        : sec.items.reduce((s, i) => s + toCNY(i.budget, i.cur), 0);
    }
